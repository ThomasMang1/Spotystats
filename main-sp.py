...
def file2data_frame(stream_file_list):
    ...
    for f_name in stream_file_list:
        with open(f"data/{f_name}", encoding="utf-8") as f:  # Set encoding to read the json file(s)
...
def main(stream_file_list):
    ...
    year = df['endTime'].dt.strftime("%Y").iloc[1]
    plot_df(df_time_sum, 'endTime', 'hrPlayed',
        title=f'Listening time to Spotify streams per day: {year} onwards',
...
if __name__ == "__main__":
    ...
    if all_files:
        ...
        print(f"No JSON file found or no file with the naming conversion Streaming_History_.json found")